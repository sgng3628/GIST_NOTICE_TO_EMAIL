...
def get_html(PD):
    ...
    driver.get(
            'https://college.gist.ac.kr/prog/bbsArticle/\
            BBSMSTR_000000005587/list.do')
    driver.find_elements_by_class_name(
            'subject')[i].find_element_by_tag_name('a').click()
...
def extract_html(html):
    ...
    title = soup.find('h2', class_='ui bbs--view--tit').get_text()
    title = " ".join(title.split())[:-2]
    date = soup.find('span', class_='date').get_text()
    content = soup.find(
            'div', class_='ui bbs--view--content').get_text().strip()
    subject = '[학내공지] (' + date[4:] + ') ' + title
    print(title)

    files = []
    for clip in soup.find_all('a', class_='btn-on-ico'):
        clip_url = 'https://college.gist.ac.kr/cmm/fms\
                /FileDown.do?atchFileId='\
                + clip['href'].split("'")[1]+'&fileSn='\
                + clip['href'].split("'")[3]
        file_name = ""
        print(clip.get_text())
        ...
        wget.download(clip_url, file_path)
        files.append(file_path)
    return subject, content, files


def send_mail(send_from, send_to, username, password, subject, message,
              files=[], server="smtp.gmail.com", port=465):
    msg = MIMEMultipart()
    msg['From'] = "GIST학내공지"
...
with requests.Session() as s:
    url = 'https://college.gist.ac.kr/prog\
          /bbsArticle/BBSMSTR_000000005587/list.do'
    notices = s.get(url)
    html = notices.text
    soup = BeautifulSoup(html, 'html.parser')
    titles = []
    ...
    if before != latest_title:
        with open(os.path.join(BASE_DIR, 'latest.txt'), 'w') as f:
            ...
            send_to = get_email('client')
            send_from = PD["my_email"]
            username = PD["my_email"]
            password = PD["my_passwd"]

            send_mail(send_from, send_to, username, password,
                      subject, message, files, "smtp.gmail.com")
